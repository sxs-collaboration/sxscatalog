...
def fetch_RIT_catalog_data():
    """ 
    Fetch metadata from the RIT Catalog to json file.

    This function scrapes the RIT Catalog page 
    and extract information about the numerical 
    simulations. The RIT Catalog can be accessed 
    through
    https://ccrg.rit.edu/numerical-simulations, 
    but the actual data is hosted at 
    https://ccrgpages.rit.edu/~RITCatalog/. The 
    function retrieves the simulation IDs, download 
    links for extrapolated Psi4 and Strain data, and 
    the metadata for each simulation. The metadata is
    parsed and stored in a structured format. When the
    script is run directly, it saves the data into a 
    JSON file specified by the user. The default name 
    for the output file is "RIT_metadata.json".

    Returns
    -------
    json file
        This json file contains a list of all the
        simulations, with their respective download
        links for Psi4, strain, and metadata and the 
        metadata itself.
    """
    from sxscatalog.metadata.metadata import Metadata
    import requests
    import tempfile
    ...
    for row in rows:
        ...
        if len(links) != 3:
            raise Exception(f"This function was expecting 3 links, but it is getting an unexpected number of links for simulation {sim_id}.")
        ...
        RITcatalogdata[sim_id]["extrap_strain_url"] = RITcatalog_url + strain_link['href']
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Fetch RIT Catalog data and save to JSON.")

    parser.add_argument("--output", type=str, default="RIT_metadata.json", help="Output JSON file name.")

    args = parser.parse_args()

    data = fetch_RIT_catalog_data()
